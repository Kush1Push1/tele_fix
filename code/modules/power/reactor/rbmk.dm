...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/fuel_rod))
		...
		radiation_pulse(get_turf(src), temperature)
		if(do_after(user, 5 SECONDS, target=src))
			...
			fuel_rods += W
			W.forceMove(src)
			radiation_pulse(get_turf(src), temperature) //Wear protective equipment when even breathing near a reactor!
...
//BLUEMOON REMOVAL		message_admins("Reactor desired criticality set to [desired_k] by [ADMIN_LOOKUPFLW(last_user)] in [ADMIN_VERBOSEJMP(src)]")
	...
	update_icon()
	radiation_pulse(get_turf(src), temperature*radioactivity_spice_multiplier)
...
/obj/effect/decal/nuclear_waste/Crossed(atom/movable/AM)
	...
	radiation_pulse(get_turf(src), 500, 5) //MORE RADS

/obj/effect/decal/nuclear_waste/attackby(obj/item/tool, mob/user)
	if(tool.tool_behaviour == TOOL_SHOVEL)
		radiation_pulse(get_turf(src), 1000, 5) //MORE RADS
